// ---------------------------【2D 描边效果】---------------------------
Shader "Custom/shader2D/outline"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                //通过tex2D对贴图位置进行采样
                //返回类型fixed4,采样返回一个颜色值，即为一个像素
                //将像素对应的rgb值赋予输出的像素颜色，将a值赋予透明度

                //总的来说，即找到贴图对应UV点，直接使用颜色值进行着色
                ...
                _lineColor *= _lineColor * _Glow;

                // if(w == 0){
                    //    col.rgb = _lineColor;
                // }
                
                //Lerp(a,b,w):根据w返回a到b之间的插值，return a+w*(b-a) 颜色渐变
                // 和原图做插值
